...
def gerar_pdf_nfse_recebida(nfse_rec, empresa) -> str:
    """Gera PDF visual para NFSe recebida (tomador). Sem itens, apenas dados do prestador."""
    pdf = FPDF(orientation="P", unit="mm", format="A4")
    pdf.set_auto_page_break(auto=True, margin=20)
    ...
    pdf.set_font("Helvetica", "B", 10)
    pdf.set_text_color(14, 165, 233)
    pdf.cell(0, 5, f"NFSe {nfse_rec.numero or 'N/D'}", align="R", new_x="LMARGIN", new_y="NEXT")
    pdf.set_font("Helvetica", "", 8)
    pdf.set_text_color(80, 80, 80)
    if nfse_rec.data_emissao:
        pdf.cell(0, 4, f"Data: {nfse_rec.data_emissao.strftime('%d/%m/%Y')}", align="R", new_x="LMARGIN", new_y="NEXT")
    if nfse_rec.chave_acesso:
        pdf.cell(0, 4, f"Chave: {nfse_rec.chave_acesso}", align="R", new_x="LMARGIN", new_y="NEXT")
    ...
    if cancelada:
        ...
        pdf.set_font("Helvetica", "B", 14)
        pdf.cell(0, 8, "CANCELADA", align="R", new_x="LMARGIN", new_y="NEXT")
    else:
        pdf.set_text_color(0, 150, 50)
        pdf.set_font("Helvetica", "B", 9)
        pdf.cell(0, 4, "AUTORIZADA", align="R", new_x="LMARGIN", new_y="NEXT")
    ...
    pdf.set_font("Helvetica", "B", 14)
    pdf.set_text_color(14, 165, 233)
    pdf.cell(0, 8, "NOTA FISCAL DE SERVICO ELETRONICA - RECEBIDA", align="C", new_x="LMARGIN", new_y="NEXT")
    pdf.set_font("Helvetica", "", 9)
    pdf.set_text_color(120, 120, 120)
    pdf.cell(0, 5, f"NFS-e {nfse_rec.numero or ''}", align="C", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(5)
    ...
    pdf.set_font("Helvetica", "B", 9)
    pdf.set_text_color(14, 165, 233)
    pdf.cell(0, 5, "PRESTADOR DO SERVICO", new_x="LMARGIN", new_y="NEXT")
    pdf.set_x(12)
    pdf.set_font("Helvetica", "", 9)
    pdf.set_text_color(40, 40, 40)
    emitente = nfse_rec.emitente_nome or "-"
    cnpj_emit = nfse_rec.emitente_cnpj or "-"
    pdf.cell(0, 5, f"{emitente} - CNPJ: {cnpj_emit}", new_x="LMARGIN", new_y="NEXT")
    pdf.set_y(y_box + 21)

    # Valor
    y_box2 = pdf.get_y()
    pdf.set_fill_color(245, 247, 250)
    pdf.rect(10, y_box2, 190, 12, "DF")
    pdf.set_xy(12, y_box2 + 2)
    pdf.set_font("Helvetica", "B", 9)
    pdf.set_text_color(14, 165, 233)
    ...
    pdf.set_text_color(40, 40, 40)
    pdf.cell(0, 5, f"R$ {float(nfse_rec.valor_total or 0):.2f}", new_x="LMARGIN", new_y="NEXT")
    pdf.set_y(y_box2 + 15)
    ...
    pdf.line(10, pdf.get_y(), 200, pdf.get_y())
    pdf.ln(3)
    data_str = nfse_rec.data_emissao.strftime('%d/%m/%Y') if nfse_rec.data_emissao else '-'
    pdf.cell(0, 3, f"Recebida em {data_str}", align="C", new_x="LMARGIN", new_y="NEXT")
    pdf.cell(0, 3, f"Chave de Acesso: {nfse_rec.chave_acesso or 'N/D'}", align="C", new_x="LMARGIN", new_y="NEXT")
    pdf.cell(0, 3, "Este documento e uma representacao visual da NFS-e recebida.", align="C", new_x="LMARGIN", new_y="NEXT")

    pdf_filename = f"nfse_rec_{nfse_rec.chave_acesso or nfse_rec.id}.pdf"
    pdf_path = os.path.join(UPLOAD_DIR, pdf_filename)
    pdf.output(pdf_path)
    return f"/static/uploads/nfse/{pdf_filename}"
