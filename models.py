...
class Empresa(Base):
    ...
    smtp_seguranca = Column(String(10), nullable=True, default="tls")
    email_auto_enviar = Column(Boolean, default=True)
    email_mensagem_padrao = Column(Text, nullable=True)
    observacao = Column(Text, nullable=True)
...
class Transportadora(Base):
    ...
    cep = Column(String(10), nullable=True)
    fornecedor_id = Column(Integer, ForeignKey("fornecedores.id"), nullable=True, index=True)
    fornecedor = relationship("Fornecedor", backref="transportadoras")
    ativo = Column(Boolean, default=True)
    created_at = Column(DateTime, default=datetime.now)
    updated_at = Column(DateTime, default=datetime.now, onupdate=datetime.now)
